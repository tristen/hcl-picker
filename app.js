  ...
  init: function(config) {
    ...
    function showGradient(from) {
      ...
      var a = gradient.handlesize,
        b = Math.floor(gradient.handlesize * 0.65),
        i, fx, fy, x, y;
    ...
    function unserialize(hash) {
      ...
      return {
        ...
        axis: parts[0],
        from: getXY(chroma(parts[3])),
        to: getXY(chroma(parts[4]))
      };
    }
...
function load() {
  ...
  mode.on('click', function() {
    ...
    if (el.attr('href').split('#')[1] === 'picker') {
      ...
      vizs.classed('hidden', false).classed('loading', false);
      chropleth(counties);
    }
  });
}
